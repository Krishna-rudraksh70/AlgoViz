...
Board.prototype.toggleTutorialButtons = function () {
  ...
  function nextPreviousClick() {
    if (counter === 1) {
      document.getElementById("tutorial").innerHTML = `<h3>Welcome to Graph Visualizer</h3>
      <h6>This short tutorial will walk you through all of the features of this application.</h6>
      <p>This is a short tutorial which will walk you through the features of the project. If you want to dive right in, feel free to press the skip button or press the next button.</p>
    ...
    `
      ...
      document.getElementById("tutorial").innerHTML = `<h3>Adding a stop</h3><h6>Click the "Add stop" button.</h6><p>Adding a stop will change the course of the chosen algorithm. In other words, the algorithm will first look for the stop (in an effort to diffuse it) and will then look for the target node. Note that the Bidirectional Swarm Algorithm does not support adding a stop.</p><img id="secondTutorialImage" src="public/styling/stop.png"><div id="tutorialCounter">${counter}/9</div><button id="nextButton" class="btn btn-default navbar-btn" type="button">Next</button><button id="previousButton" class="btn btn-default navbar-btn" type="button">Previous</button><button id="skipButton" class="btn btn-default navbar-btn" type="button">Skip Tutorial</button>`
    } else if (counter === 7) {
